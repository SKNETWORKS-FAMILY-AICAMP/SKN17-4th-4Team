...
| ID           | 시나리오        | 절차                    | 기대 결과                          |
| ------------ | ----------- | --------------------- | ------------------------------ |
| TC-USER-01  | 이메일 로그인 성공  | 올바른 계정 입력 → 로그인       | 홈 이동, 세션 생성                    |
| TC-USER-02  | 이메일 로그인 실패  | 잘못된 비밀번호 입력           | 오류 토스트(3-1) 노출                 |
| TC-USER-01 | 인증 메일 발송/검증 | 이메일 입력 → 인증요청 → 코드 입력 | 1분 내 메일 수신·검증 통과               |
...
## 12. 한줄 회고록
* 김민균: 프로젝트에서 피그마등을 새롭게 접근을 하게 되었고 AWS등을 처음 하게 되어 보면서 어려운것도 많이 있었고 시간이 부족하여서 이번 프로젝트에서 추가를 하고 싶었지만 못 하였던 부분들이 너무 아쉽게 느껴지게 되었습니다. 다음에는 조금 더 노력을 하여서 부족한 부분들을 채워서 프로젝트를 좀더 완벽하게 만들고 싶습니다.
* 김세한:
* 김수현: 이번 프로젝트에서 아키텍처 설계, Figma 화면 설계, Django 환경 구축을 담당했습니다. Figma를 처음 사용해 어려움도 있었지만, 디자인을 구현하는 과정이 흥미로웠습니다. Django와 AWS를 다루며 이론과 실제의 차이를 느꼈고, 배포까지 완성하는 웹 개발에 도전하고 싶다는 목표가 생겼습니다.
* 정의중:
